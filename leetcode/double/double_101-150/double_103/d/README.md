...
设上一个被删除的数的位置为 {pre}（初始为 1），当前需要删除的位置为 i。定义 text{query}(i,j) 表示下标在 [i,j] 中的被删除的元素个数。考虑从 {pre} 到 i 的移动次数：

- 如果 {pre}le i，那么移动次数等于 i-{pre}-text{query}({pre},i)。
- 如果 {pre}> i，那么移动次数等于 (n-{pre}-text{query}({pre},n))+(i-text{query}(1, i))。假设删除了 k 个数，那么 text{query}({pre},n))+text{query}(1,i) = k - text{query}(i+1,{pre}-1)=k - text{query}(i,{pre}-1)，因为 i 处的元素还没被删除。所以上式最终化简为 i-{pre}+n-k+text{query}(i,{pre}-1)。

代码实现时，text{query}(i,j) 可以用树状数组实现。（也可以用线段树、名次树等数据结构。）
...
- 时间复杂度：{O}(nlog n)，其中 n 为 {nums} 的长度。瓶颈在排序上。
- 空间复杂度：{O}(n)。
